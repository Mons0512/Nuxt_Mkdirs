...
const { data: groupsData, pending, refresh, error } = await useFetch(
  '/api/admin/groups',
  {
    query: computed(() => ({
...
const totalPages = computed(() => groupsData.value?.totalPages || 1);
  ...
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-gray-500">Manage product groups</p>
      </div>
      <button
        @click="openModal()"
    ...
    <div v-else-if="error" class="text-center py-12 text-red-600">Failed to load groups</div>
        ...
        <table class="min-w-full">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="group in groups" :key="group.id" class="hover:bg-gray-50">
              <td class="px-4 py-4 text-sm font-medium text-gray-900">{{ group.name }}</td>
              <td class="px-4 py-4 text-sm text-gray-500">{{ group.slug }}</td>
              <td class="px-4 py-4 text-sm text-gray-500">
                {{ group.description ? group.description.slice(0, 50) + (group.description.length > 50 ? '...' : '') : '-' }}
              </td>
              <td class="px-4 py-4 text-sm text-gray-500">{{ group.priority }}</td>
      ...
      <div class="bg-white rounded-xl w-full max-w-md">
        ...
        <form @submit.prevent="saveGroup" class="p-5 space-y-4">
          ...
          <div>
            ...
            <input
              v-model.number="editingGroup.priority"
              type="number"
              class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gray-200 focus:border-gray-400 outline-none transition-all"
            />
          </div>
