...
const { data: tagsData, pending, refresh, error } = await useFetch(
  '/api/admin/tags',
  {
    query: computed(() => ({
...
const total = computed(() => tagsData.value?.total || 0);
const totalPages = computed(() => tagsData.value?.totalPages || 1);
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 mb-1">Tags</h1>
        <p class="text-sm text-gray-500">Manage product tags</p>
      </div>
      <button
        @click="openModal()"
    ...
    <div v-else-if="error" class="text-center py-12 text-red-600">Failed to load tags</div>
        ...
        <table class="min-w-full">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="tag in tags" :key="tag.id" class="hover:bg-gray-50">
              <td class="px-4 py-4 text-sm font-medium text-gray-900">{{ tag.name }}</td>
              <td class="px-4 py-4 text-sm text-gray-500">{{ tag.slug }}</td>
              <td class="px-4 py-4 text-sm text-gray-500">
                {{ tag.description ? tag.description.slice(0, 50) + (tag.description.length > 50 ? '...' : '') : '-' }}
              </td>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl w-full max-w-md">
        ...
        <form @submit.prevent="saveTag" class="p-5 space-y-4">
          <div>
